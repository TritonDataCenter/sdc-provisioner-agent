...
ProvisionerAgent.prototype.addAuthorizedKeys = function (msg) {
  ...
  appendToFile(
    ...
    , function (error) {
        if (error) {
          return self.ackError(msg.id,
            "Error writing authorized_keys file: " +
            error.toString());
        }
...
ProvisionerAgent.prototype.provision = function () {
  ...
  self.configureZone(env, function (zone_xml) {
    self.configureZFS(env, function (error) {
      if (error) {
        self.ackError(self.msg.id, "ZFS error: " + error.stderr);
        return;
      }
...
ProvisionerAgent.prototype.followZoneinitServiceLog = function (env, logs) {
  ...
  fs.watchFile(logPath, { interval: 500 }, function (curr, prev) {
    puts ("watch args" + inspect(arguments));
    ...
    fs.readFile(logPath, function (error, data) {
      puts("Read log file" + inspect(arguments));
