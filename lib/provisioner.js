...
ProvisionerAgent.prototype.isDone = function () {
  console.log("Current provisions: " + this.provisionCount);
...
ProvisionerAgent.prototype.setupProvisionQueue = function () {
  ...
  self.provisionQueue
    = self.connection.queue
      ( self.config.resource + '-provisionz.' + self.uuid, { autoDelete: true });
...
ProvisionerAgent.prototype.handleProvision = function (msg) {
  ...
  function shouldShift () {
    ...
    else if (self.gracefulStop) {
      console.log("Not shifting because we're shutting down.");
      return false;
    }
    else if (!(self.provisionCount < self.config.maxConcurrentProvisions)) {
      console.log("Not shifting because we're at the maximum number of concurrent provisions (" + self.provisionCount + ").");
      return false;
    }
    else {
