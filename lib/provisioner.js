...
ProvisionerAgent.prototype.setupProvisionQueue = function () {
  ...
  self.provisionQueue
    = self.connection.queue
      ( self.config.resource + '-provisions.' + self.hostname
      , { durable: true
        , autoDelete: false
        , exclusive: true
        });
...
ProvisionerAgent.prototype.add_authorized_keys = function (msg) {
  ...
  var thePath
    = path.join
        ( self.config.zonesZpoolPath
        , msg.data.zonename
        , 'root/home/' + username + '/.ssh/authorized_keys'
        );
  ...
  function verifyFileMagic() {
    fileMagic(thePath, function (error, fileType) {
      if (error) {
        var errorMsg
          = "The was a problem looking up the authorized_keys file type: " +
            error.toString();
        self.ackError(msg.id, errorMsg);
        self.provisionerEvent('error', msg.data.zonename, msg.id, errorMsg);
        return;
      ...
      if (['ascii text', 'data'].indexOf(fileType) === -1) {
        var errorMsg = "The authorized_keys file was not a plain file.";
        self.ackError(msg.id, errorMsg);
        self.provisionerEvent('error', msg.data.zonename, msg.id, errorMsg);
        return;
      }
...
ProvisionerAgent.prototype.handleProvision = function (msg) {
  ...
  if (self.provisionCount < self.config.maxConcurrentProvisions) {
...
ProvisionerAgent.prototype.followZoneinitServiceLog = function (env, logs, callback) {
  ...
  fs.watchFile(logPath, { interval: 500 }, function (curr, prev) {
    ...
    puts("Log file for zone '" + env.zonename + "' has changed. Checking...");
