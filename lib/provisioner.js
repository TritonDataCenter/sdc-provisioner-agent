...
ProvisionerAgent.prototype.add_authorized_keys = function (msg) {
  ...
  function verifyFileOwnership() {
    getFileOwner(thePath, function (error, keysOwner) {
      if (error) {
        self.ackError(msg.id, "error looking up authorized_keys permissions:" + error.toString());
        return;
      }
      console.dir(keysOwner);
      getFileOwner(homePath, function (error, homeOwner) { 
        if (error) {
          self.ackError(msg.id, "error looking up authorized_keys permissions: " + error.toString());
          return;
        }
      console.dir(homeOwner);
        var keysStr = keysOwner.join('/');
        var homeStr = homeOwner.join('/');

        if (keysStr !== homeStr) {
          self.ackErro(msg.id, "authorized_keys owner did not match home directory owner");
          return;
        }
