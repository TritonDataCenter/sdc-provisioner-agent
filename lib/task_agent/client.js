...
AgentHandle.prototype.prepareAgentEventQueue = function (callback) {
    ...
    this.log.info('Waiting for task events on queue: ' + queueName);
        ...
        queue.subscribe(function (msg, headers, deliveryInfo) {
            ...
            if (self.taskHandles[taskId]) {
                ...
                if (eventType === 'finish') {
                    ...
                    self.taskHandles[taskId].removeAllListeners();
...
AgentHandle.prototype.sendTask = function (task, msg, callback) {
    ...
    self.log.debug('Sending task to %s', self.uuid);
    ...
    taskHandle.timeout = setTimeout(function () {
        self.log.error(
            ...
            task, self.uuid, self.commandTimeout * 1000);
        self.taskHandles[msg.task_id].removeAllListeners();
