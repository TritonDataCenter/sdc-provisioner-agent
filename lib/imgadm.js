        ...
        function (err, stdout, stderr) {
            ...
            if (err) {
                ...
                options.log.warn(format(
                    'Error creating VM image:\n'
                    + '\targv: %j\n'
                    + '\texit status: %s\n'
                    + '\terr: %s\n'
                    + '\tstdout: %s\n'
                    + '\tstderr: %s', options.zpool, argv, err.code,
                    String(err).trim(), stdout.trim(),
                    stderr.trim()));

                callback(new Error(errlines[errlines.length]));
                return;
            }
            options.log.info(format('imported image %s: stdout=%s stderr=%s',
                options.uuid, stdout.trim(), stderr.trim()));
