...
ProvisionTask.prototype.fetchDatasetIfMissing = function (env, dataset, callback) {
  ...
  function onList (error, fields, list) {
    ...
    function datasetNameVersion(dataset) {
      var dashIdx = dataset.lastIndexOf('-');
      var name = dataset.slice(0, dashIdx);
      var version = dataset.slice(1+dashIdx);
      return { name: name, version: version };
    }

    self.queryDatasetAPI(datasetNameVersion(dataset), function (error, response) {
...
ProvisionTask.prototype.queryDatasetAPI = function (query, callback) {
  ...
  var options
    = { host: self.agent.sdcConfig['assets_admin_ip']
      ...
      , path: '/datasets?' + qs.stringify(query)
      };
