...
Client.prototype.getAgentHandle = function (hostname, resource, callback) {
  var handle;
  ...
  if (this.agentHandles[hostname]) {
    handle = this.agentHandles[hostname];
  }
  else {
    handle = this.agentHandles[hostname]
...
AgentHandle.prototype.prepareAckQueue = function(callback) {
  ...
  var queueName = this.resource + '.req.' + this.hostname + '.' + genId();
  puts('Awaiting ACK on queue: ' + queueName);

  var queue = this.connection.queue(queueName, { autoDelete: true }, function () {
    queue.bind('amq.topic', self.resource + '.ack' + self.clientId + '.' + self.hostname);
...
AgentHandle.prototype.listenForAck = function(id, callback) {
  ...
  function onTimeout() {
    ...
    var msg = { error: "Timed out waiting for response from dataset agent "
...
AgentHandle.prototype.sendCommand = function(commandName, payload, callback) {
  var self = this;
  var routingKey = this.resource + "." + commandName + "." + this.hostname;
  ...
  // Listen for an ACK in reply to message with `id` from this hostname
  // and set a timeout
